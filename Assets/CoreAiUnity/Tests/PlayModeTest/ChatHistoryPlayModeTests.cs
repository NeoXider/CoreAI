...
{
    /// <summary>
    /// PlayMode тест: Проверяем, что ChatHistory правильно сохраняется на диск (PersistentChatHistoryBetweenSessions = true)
    /// и восстанавливается при создании нового оркестратора (имитация перезапуска игры).
    /// </summary>
#if !COREAI_NO_LLM
    public sealed class ChatHistoryPlayModeTests
    {
        ...
        public void Setup()
        {
            // Очищаем директорию перед тестом, просто на всякий случай
            // Так как FileAgentMemoryStore использует жесткий путь, нам нужно его очистить для этого конкретного roleId
        ...
        public IEnumerator ChatHistory_PersistentBetweenSessions_Works()
        {
            ...
            try
            {
                ...
                AgentConfig chatAgent = new AgentBuilder("TestPersistentChatAgent")
                    ...
                    .WithMemory(MemoryToolAction.Append) // включаем память
                    .WithChatHistory(8192, true) // ВАЖНО: persistent = true
                ...
                // === СЕССИЯ 2 === (полностью новые инстансы)
