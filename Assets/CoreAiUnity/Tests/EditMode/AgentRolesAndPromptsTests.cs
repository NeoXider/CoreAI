...
namespace CoreAI.Tests.EditMode
{
    public sealed class AgentRolesAndPromptsTests
    {
        ...
        public void AiPromptComposer_AllBuiltInRoles_GetSystemPrompt()
        {
            ...
            var composer = new AiPromptComposer(provider, noUser, new NullLuaScriptVersionStore());
            foreach (var role in BuiltInAgentRoleIds.AllBuiltInRoles)
            {
        ...
        public void AiPromptComposer_UserTemplate_ReplacesTelemetryPlaceholders()
        {
            ...
            var user = new FixedUserTemplateProvider("w={wave} m={mode} hint={hint}");
            var composer = new AiPromptComposer(sys, user, new NullLuaScriptVersionStore());
            var snap = new GameSessionSnapshot();
            snap.Telemetry["wave"] = "7";
            snap.Telemetry["mode"] = "solo";
            var u = composer.BuildUserPayload(snap, new AiTaskRequest { RoleId = BuiltInAgentRoleIds.Creator, Hint = "go" });
        ...
        private sealed class FixedUserTemplateProvider : IAgentUserPromptTemplateProvider
        {
            ...
            public FixedUserTemplateProvider(string template) => _template = template;
