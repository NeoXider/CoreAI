...
namespace CoreAI.Infrastructure.Llm
{
    ...
    public sealed class CoreAISettingsAsset : ScriptableObject
    {
        ...
        public static void SetInstance(CoreAISettingsAsset settings) => _instance = settings;
        ...
        [Tooltip("Приоритет в Auto режиме: LLMUnity сначала или HTTP API сначала.")]
        [SerializeField]
        private LlmAutoPriority autoPriority = LlmAutoPriority.LlmUnityFirst;

        [Header("🌐 HTTP API (OpenAI-compatible)")]
        [Tooltip("База URL без завершающего слэша: https://api.openai.com/v1, http://localhost:1234/v1 (LM Studio), http://localhost:5001/v1 (Qwen) и т.д.")]
        [SerializeField]
        ...
        [Tooltip("Название модели: gpt-4o-mini, qwen3.5-4b, llama-3-8b и т.д.")]
        [SerializeField]
        ...
        [Tooltip("Таймаут HTTP-запроса в секундах. 0 = без таймаута.")]
        [SerializeField]
        [Min(0)]
        ...
        [Tooltip("Путь к .gguf модели. Пусто = автоопределение через Model Manager.")]
        [SerializeField]
        private string ggufModelPath = "Qwen3.5-2B-Q4_K_M.gguf";

        [Tooltip("Не уничтожать LLMUnity GameObject при смене сцены.")]
        [SerializeField]
        private bool llmUnityDontDestroyOnLoad = true;

        [Tooltip("Таймаут запуска LLMUnity сервиса (секунды).")]
        [SerializeField]
        [Min(5f)]
        private float llmUnityStartupTimeoutSeconds = 120f;

        [Tooltip("Задержка после запуска LLMUnity сервиса (секунды).")]
        [SerializeField]
        [Min(0f)]
        private float llmUnityStartupDelaySeconds = 1f;

        [Tooltip("Не останавливать LLMUnity сервер между запросами (keep-alive). Ускоряет тесты.")]
        [SerializeField]
        private bool llmUnityKeepAlive = false;

        [Tooltip("Максимум параллельных чатов с LLMUnity. 1 = последовательно.")]
        [SerializeField]
        [Min(1)]
        ...
        [Tooltip("Контекстное окно по умолчанию (токены).")]
        [SerializeField]
        [Min(256)]
        ...
        [Tooltip("Текст который будет возвращаться вместо всех запросов.")]
        [SerializeField]
        [TextArea(3, 8)]
        private string offlineCustomResponse = "Offline mode: LLM unavailable";

        [Tooltip("Для каких ролей использовать кастомный ответ (* = все).")]
        [SerializeField]
        private string offlineCustomResponseRoles = "*";

        [Header("🔧 Отладка")]
        [Tooltip("Включить подробное логирование MEAI pipeline.")]
        [SerializeField]
        private bool enableMeaiDebugLogging = false;

        [Tooltip("Логировать сырые HTTP request/response (для отладки API).")]
        [SerializeField]
        ...
        [Tooltip("Логировать количество токенов (input, output, total) в каждом ответе.")]
        [SerializeField]
        private bool logTokenUsage = true;

        [Tooltip("Логировать время отклика LLM (latency в миллисекундах).")]
        [SerializeField]
        private bool logLlmLatency = true;

        [Tooltip("Логировать ошибки подключения к LLM (timeout, network error).")]
        [SerializeField]
        ...
        [Tooltip("Максимум параллельных задач оркестратора.")]
        [SerializeField]
        [Min(1)]
        private int maxConcurrentOrchestrations = 2;

        [Tooltip("Логировать метрики оркестратора.")]
        [SerializeField]
        ...
        public string OfflineCustomResponse => string.IsNullOrWhiteSpace(offlineCustomResponse) ? "Offline mode: LLM unavailable" : offlineCustomResponse;

        /// <summary>Для каких ролей использовать кастомный ответ (* = все).</summary>
        public string OfflineCustomResponseRoles => string.IsNullOrWhiteSpace(offlineCustomResponseRoles) ? "*" : offlineCustomResponseRoles;
        ...
        {
            if (!offlineUseCustomResponse) return false;
            if (offlineCustomResponseRoles == "*") return true;
            if (string.IsNullOrEmpty(roleId)) return false;
            var roles = offlineCustomResponseRoles.Split(',');
            foreach (var r in roles)
            {
                if (r.Trim().Equals(roleId, System.StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
        ...
        public string ApiBaseUrl => string.IsNullOrWhiteSpace(apiBaseUrl) ? "http://localhost:1234/v1" : apiBaseUrl.TrimEnd('/');
        ...
        public float LlmUnityStartupTimeoutSeconds => llmUnityStartupTimeoutSeconds < 5f ? 120f : llmUnityStartupTimeoutSeconds;
        ...
        private void OnValidate()
        {
            ...
            if (llmRequestTimeoutSeconds < 0f) llmRequestTimeoutSeconds = 15f;
            if (llmUnityStartupTimeoutSeconds < 5f) llmUnityStartupTimeoutSeconds = 120f;
            if (llmUnityStartupDelaySeconds < 0f) llmUnityStartupDelaySeconds = 1f;
            if (llmUnityMaxConcurrentChats < 1) llmUnityMaxConcurrentChats = 1;
        }
#endif
