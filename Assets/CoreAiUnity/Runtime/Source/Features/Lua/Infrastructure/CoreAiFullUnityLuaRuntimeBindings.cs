...
namespace CoreAI.Infrastructure.Lua
{
    ...
    public sealed class CoreAiFullUnityLuaRuntimeBindings : IGameLuaRuntimeBindings
    {
        ...
        private static int GetObjectId(UnityEngine.Object obj)
        {
            ...
            // GetInstanceID() returns a stable, session-unique int — exactly what Resolve() needs (no
            // GetHashCode() collisions). Unity 6.5 made the EntityId->int implicit cast an obsolete ERROR
            // (CS0619: "EntityId will not be representable by an int in the future"), so we no longer route
            // through GetEntityId() here.
