...
- **Verified**: on a WebGL/IL2CPP player at **Medium** stripping the VContainer registrations, `RbxWorldHost` binding, bundled-mod seeding, the Lua VM and mod-driven `Instance.new` spawning all survive on the shipped `link.xml`. Your own DI/reflection-resolved assemblies still need entries in your project's own `link.xml`.
- **Windows Standalone currently ships Mono2x.** IL2CPP for Windows needs the Visual Studio **Desktop development with C++** workload (MSVC toolset) plus **Windows SDK 10.0.19041+**; without them the build fails with `Unity.IL2CPP.Bee.BuildLogic.ToolchainNotFoundException` ("Visual Studio installation without C++ tool components"). Android already ships IL2CPP/arm64-v8a, so the AOT path is exercised regardless.
