...
Without Self-Service Skills, every tool's full JSON schema rides in *every* request: the root `README.md` estimates **50 tools across 10 skills ≈ ~4,000 tokens** of definitions. With skills, the model sees a lightweight catalog plus **2 constant meta-tools** (`read_skill`, `call_skill_tool`) — **~360 tokens, a ~91% saving** — and loads full schemas only for the skill it actually uses. If your agents carry more than a few tools, attaching them as skills is the single biggest input-token lever you have. `call_skill_tool` checks the arguments before the skill tool runs: a missing, `null` or mistyped argument is refused with the expected parameter list and a retry instruction, so a malformed call costs one extra roundtrip instead of a failed side effect.
...
Note: when a backend returns no `usage` (common for streaming and for LLMUnity's local server) the `LLM <` log line shows `tokens n/a` — local calls cost nothing per token anyway; only paid HTTP paths need accounting.
...
- **F10 token-budget overlay** — `CoreAiTokenBudgetOverlay` (IMGUI drop-in, toggle **F10**) and `CoreAiTokenBudgetUiView` (for your own Canvas) show live token usage, request-load/rate-limiter saturation, and a **$/session estimate** driven by two fields on `CoreAISettingsAsset`: **Input $ / 1K Tokens** and **Output $ / 1K Tokens** (Advanced Settings → **Debug** tab; `0` = show tokens only). Type your provider's current prices into those two fields and the overlay does session cost math for you during playtests.

### Practical measuring loop

1. Enter your provider's per-1K prices in the settings asset (Advanced Settings → Debug tab).
...
| Lever | Where | Effect |
|---|---|---|
| **Per-agent / global `MaxOutputTokens`** | `CoreAISettingsAsset` General settings — since 5.9.0 the global cap is an explicit override (**Override Max Output Tokens**) that is **OFF by default** (no `max_tokens` sent, provider decides); turn it on and set the field (default 128000 — effectively uncapped; never set a tight budget, reasoning models burn it on thinking) to cap globally, or override per agent `AgentBuilder.WithMaxOutputTokens`, per call `AiTaskRequest.MaxOutputTokens` | Hard-caps output tokens per request. Rule of thumb: do not cap at all, or cap at 128000 — tight budgets silently truncate reasoning models (thinking counts toward `max_tokens`) and look like model failures. Control cost with cheaper models and shorter prompts, not output caps. |
| **History budget + summarization** | `Recent history token budget override`, `Enable history summarization`, compaction trigger ratio, context pruning, `Max retained tool results` (Advanced Settings → Memory tab) | Bounds the input-side history tail; older turns fold into a compact rolling summary instead of being re-sent verbatim forever. |
| **Context Window setting** | `CoreAISettingsAsset` General settings | Since 5.9.0 the client-side window is an explicit override (**Override Context Window**) that is **OFF by default** — budgeting is effectively unlimited (`UnlimitedContextWindowTokens`) and the provider enforces its real limit. Turn the override on and set the field (default 131072) to make the automatic history budget derive from a bounded window; set it to what the *feature* needs, not what the model supports. |
...
| **Tool-loop caps** | `maxToolCallRoundtrips` (default 20; serialized, not in the custom inspector — set it in code or per agent), **Tool Call Retries**, **Lua Repair Retries** | Bounds the number of billed LLM calls a single player action can trigger. Note built-in `Programmer`, `Creator` and `Builder` default to unlimited roundtrips — set explicit caps for production cloud builds. |
...
| **Retry budget** | `maxLlmRequestRetries` (default 1; serialized, not in the custom inspector) | Each automatic retry is a billed request; keep it low for expensive models. No retry runs after the caller cancelled. |
