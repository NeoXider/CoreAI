...
| Lever | Where | Effect |
|---|---|---|
| **Per-agent / global `MaxOutputTokens`** | `CoreAISettingsAsset` General settings (global, default 4096); override per agent `AgentBuilder.WithMaxOutputTokens`, per call `AiTaskRequest.MaxOutputTokens` | Hard-caps output tokens per request. Chatty NPCs rarely need more than 256–512; background analyzers even less. Output tokens are usually the *expensive* ones — cap them first. |
...
| **Context Window setting** | `CoreAISettingsAsset` General settings | The automatic history budget derives from the context window; a 128K window invites 128K-sized prompts. Set it to what the *feature* needs, not what the model supports. |
