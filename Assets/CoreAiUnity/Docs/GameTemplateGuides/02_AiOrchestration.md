...
**Current code map:** [DEVELOPER_GUIDE.md §3](../DEVELOPER_GUIDE.md) (flow `RunTaskAsync` → `ILlmClient` + tools (`execute_lua`, `world_command`, …) → `ApplyAiGameCommand` → world commands / `CommandReceived`).
...
- Model calls: the container resolves **`ILlmClient`** as `TimeoutLlmClientDecorator` → **`LoggingLlmClientDecorator`** (portable, `CoreAI.Core`; uses **`ILog`** instead of `IGameLogger`) → `RetryingStreamingLlmClientDecorator` → `RoutingLlmClient`; inside: `OpenAiChatLlmClient` (HTTP APIs and LLMUnity's local server) / stub; timeout and **`[Llm]`** logs — see [DEVELOPER_GUIDE §3–4](../DEVELOPER_GUIDE.md), [LLMUNITY_SETUP_AND_MODELS §1](../LLMUNITY_SETUP_AND_MODELS.md). Streaming is the default execution path: when **`ICoreAISettings.EnableStreaming`** is on, `RunTaskAsync` obtains each completion via **`CompleteStreamingAsync`** (through a `CompleteForTaskAsync` helper) so non-interactive tasks share the execute-as-you-stream tool path with chat; it falls back to non-streaming **`CompleteAsync`** only when streaming is disabled.
...
- After the model responds: **`ApplyAiGameCommand`** (`AiEnvelope` + **`TraceId`**). Lua runs during the turn through the `execute_lua` tool; **`LuaExecutionSucceeded`** / **`LuaExecutionFailed`** are published only by the opt-in `LuaCsAiEnvelopeProcessor` if your host registers it.
