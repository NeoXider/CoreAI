...
| **Max Output Tokens** | `2048` | Global LLM response token limit — applied uniformly to **both** HTTP API and LLMUnity **only when the override above is on**. Per-agent override: `AgentBuilder.WithMaxOutputTokens`. Per-call override: `AiTaskRequest.MaxOutputTokens`. Per-request override: `LlmCompletionRequest.MaxOutputTokens`. With the override off, `ICoreAISettings.MaxTokens` resolves to `0` (no cap / provider default). |
