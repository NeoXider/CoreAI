...
| **LLM Timeout** | `15` | LLM request timeout (seconds). Enforced by `CoreAiChatService` as an idle window: a PlayerLoop-driven watchdog re-armed by every streamed chunk and tool-call event (WebGL-compatible). |
