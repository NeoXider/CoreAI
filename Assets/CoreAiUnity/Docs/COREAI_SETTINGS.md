...
| **Temperature** | `0.2` | 0.0 = deterministic, 2.0 = creative. Whether this value is sent on the wire is governed by **General → Override temperature** (since **v1.7.0**): when override is off, the HTTP client omits the JSON **`temperature`** field so the provider default applies. **`ConfigureHttpApi`** enables the override so programmatic HTTP setup still sends temperature. |
...
| Field | Default | Description |
|------|-------------|----------|
| **Temperature** | `0.1` | Shared slider value (0.0 = deterministic, 2.0 = creative). Sent to HTTP + LLMUnity only when **Override temperature** is **on** (`LlmCompletionRequest.SendTemperature`); when **off**, backends use their default sampling (**v1.7.0+**). |
| **Override temperature** | ❌ | When **off** (default), **`temperature`** is not sent (OpenAI-compatible JSON omits the key; MEAI **`ChatOptions.Temperature`** is unset). When **on**, the **Temperature** slider applies globally. **`ConfigureHttpApi`** sets override **on** so code-driven HTTP setup matches legacy behaviour. |
