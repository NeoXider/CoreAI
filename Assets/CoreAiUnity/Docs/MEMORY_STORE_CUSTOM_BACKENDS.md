...
Three consequences a custom backend has to plan for. **A resend of an unanswered message is not recorded twice** (since 7.44.0): before appending, the orchestrator reads `GetChatHistory(roleId, 1)`, and when the history already ends with a `user` message whose content is byte-identical to this turn's (raw hint plus attachment placeholders), that record is taken to be this turn — nothing is appended, and the copy is also dropped from the request's history so the model does not read the question twice. A user message at the tail is unanswered by construction (an answered turn appends its assistant message after it), which is why the rule needs no marker and survives a reload. The decision is taken once per request, from the raw store tail, and both the prompt and the teardown append follow it (pruning a tool message out of the prompt cannot turn an answered turn into an "unanswered" one on one side only). The rule is deliberately narrow: an identical message sent after an answer, a different message after a cancelled one, and any turn with attachments (history keeps only a name/type/size descriptor, so two different images can describe identically) are recorded as usual. A backend must therefore return the **latest** messages, oldest first, for `maxMessages > 0` (the `GetChatHistory` contract); if that read throws, the append happens anyway (a duplicate is cheaper than losing the learner's words). Removing the record when a turn is cancelled was rejected — it would reopen the 7.41.0 hole where the question stayed on screen but the model never learned it was asked. **An `AppendChatMessage` that throws during failure teardown loses that message** — the store error is logged as a warning and deliberately not allowed to replace the cancellation or provider failure the turn is already reporting. **Size the chat cap against the fold window, not against a single turn**: an append evicts the oldest message whether or not this turn managed to commit its summary, so a cap close to the message count that triggers folding can evict source nothing has retold yet. The shipped stores trim at 500 messages while folding starts around thirty, which is what makes the occasional broken turn affordable.
