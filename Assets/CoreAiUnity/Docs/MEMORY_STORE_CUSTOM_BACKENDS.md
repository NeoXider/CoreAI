...
CoreAI treats **MemoryTool** (long-term `memory` string) and **ChatHistory** (session dialogue) through one contract: [`IAgentMemoryStore`](../../CoreAI/Runtime/Core/Features/AgentMemory/IAgentMemoryStore.cs). The default Unity implementation is [`FileAgentMemoryStore`](../Runtime/Source/Features/AgentMemory/Infrastructure/FileAgentMemoryStore.cs) (under `Application.persistentDataPath/CoreAI/AgentMemory/`, layout v2: `<stem>.json` for the memory document and `<stem>.history.jsonl` for the conversation, one JSON line per chat/transcript record; `<stem>` is the role id or `scope-v1-<sha256>`). **Editor:** to wipe **all** CoreAI persistence under `persistentDataPath/CoreAI` at once (memory, summaries, version stores), use **CoreAI → Delete All Persistent Saves...** (not during Play Mode). **WebGL:** this store is supported — `persistentDataPath` is backed by the browser (IndexedDB / Unity virtual FS); respect storage quotas and expect weaker guarantees in private browsing mode.
...
**Important:** `Clear` clears **MemoryTool** state; `ClearChatHistory` clears **only** chat. On disk, `FileAgentMemoryStore` keeps these in separate files (`<stem>.json` and `<stem>.history.jsonl`) so one can be wiped without erasing the other (see EditMode tests `FileAgentMemoryStoreEditModeTests`). If your store caches chat in RAM per `roleId`, **`ClearChatHistory` must invalidate that cache** (or equivalent) so the next `GetChatHistory` reflects durable storage — the reference `FileAgentMemoryStore` resets its in-memory list and reload flag for the role when chat is cleared.
...
- Blob holds the same logical fields as `FileAgentMemoryStore`: `memory` and `lastSystemPrompt` (its `<stem>.json` document) plus the chat messages (its `<stem>.history.jsonl` records), so you can copy merge logic from the reference implementation. The legacy `chatHistoryJson` / `transcriptEntriesJson` fields are only read to migrate old files.
...
Three consequences a custom backend has to plan for. **A resend of an unanswered message is not recorded twice:** the orchestrator decides it from the **same single `GetChatHistory` read** that builds the prompt (no extra read per request). When the history already ends with a `user` message whose content is byte-identical to this turn's (raw hint plus attachment placeholders), that record is taken to be this turn — nothing is appended, and the copy is also dropped from the request's history so the model does not read the question twice. A user message at the tail is unanswered by construction (an answered turn appends its assistant message after it), which is why the rule needs no marker and survives a reload. The decision is taken once per request, from the raw store tail of that read (its last item, whatever the cap), and both the prompt and the teardown append follow it (pruning a tool message out of the prompt cannot turn an answered turn into an "unanswered" one on one side only). A separate one-message read, `GetChatHistory(roleId, 1)`, remains only on teardown paths that never built a request (authority denied, a queue that terminated admitted work). The rule is deliberately narrow: an identical message sent after an answer, a different message after a cancelled one, and any turn with attachments (history keeps only a name/type/size descriptor, so two different images can describe identically) are recorded as usual. A backend must therefore return the **newest** messages, newest last, for `maxMessages > 0` (the `GetChatHistory` contract). If the teardown tail read throws, the append happens anyway (a duplicate is cheaper than losing the learner's words); if the main history read throws, the request is not built and that same teardown path records the user message. Removing the record when a turn is cancelled was rejected — it would reopen the 7.41.0 hole where the question stayed on screen but the model never learned it was asked. **An `AppendChatMessage` that throws during failure teardown loses that message** — the store error is logged as a warning and deliberately not allowed to replace the cancellation or provider failure the turn is already reporting. **Size the chat cap against the fold window, not against a single turn**: an append evicts the oldest message whether or not this turn managed to commit its summary, so a cap close to the message count that triggers folding can evict source nothing has retold yet. The shipped stores trim at 500 messages while folding starts around thirty, which is what makes the occasional broken turn affordable.
