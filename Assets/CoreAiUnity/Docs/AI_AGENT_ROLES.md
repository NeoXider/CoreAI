...
### 6.6 Стек Qwen 3.5 (2B / 4B / 9B) в LLMUnity

- **2B** — минимальная модель для строгого JSON и коротких задач (**CoreMechanicAI**, батч **Analyzer**) при хорошем системном промпте.
- **4B** — рекомендуемый баланс для **AINpc**, отчётов и большинства задач без огромного прироста веса.
