...
### 1.3.5 Универсальный системный промпт (Universal System Prompt Prefix)

Начиная с **v0.11.0**, CoreAI поддерживает **универсальный стартовый промпт** — текст, который добавляется в **НАЧАЛО** системного промпта **каждого** агента (встроенного и кастомного).
...
**Где настроить:**
- **Inspector:** `CoreAISettings` → секция "⚙️ Общие настройки" → "Universal System Prompt Prefix"
- **Код:** `CoreAISettings.UniversalSystemPromptPrefix = "..."`

**Как работает:**
```
Финальный системный промпт = UniversalPrefix + " " + AgentSpecificPrompt
```
...
**Применение:**
| Агент | Итоговый промпт |
|-------|----------------|
| Creator | "**You are an AI agent...** You are the Creator agent..." |
| Programmer | "**You are an AI agent...** You are the Programmer agent..." |
| Кастомный (AgentBuilder) | "**You are an AI agent...** You are a storyteller..." |

> ⚠️ Префикс **не заменяет** специфичные промпты агентов — он только дополняет их. Каждая роль по-прежнему имеет свой уникальный системный промпт.
...
- Имена вроде **Qwen3 / Qwen2.5 2B** быстро устаревают; ниже — **классы размера** и тип задачи. Конкретную чекпойнт-модель подбирайте под **JSON/structured output** и **латентность** на вашем железе.
...
|------|------------------|-------------------------------|----------------------------------|---------|
| **CoreMechanicAI** | Числа, флаги, короткий JSON (статы, шанс, «сломать/нет») | **Tiny достаточно** при узкой схеме и хорошем промпте — в духе **Qwen ~1B–1.5B** или аналог | Граф несовместимостей «как химия», длинные цепочки модификаторов → **Small–Medium** | Часто выгодно **кэшировать** типовые исходы; LLM только для «редких» комбинаций. |
...
### 6.6 Стек Qwen 3.5 (0.8B / 2B / 4B / 9B) в LLMUnity

- **0.8B** — самая маленькая модель. Большинство PlayMode тестов проходит (memory, tool calling, Lua). Сложности только с многошаговыми цепочками (multi-step tool calls).
- **2B** — работает, но иногда ошибается в многошаговых сценариях. Золотая середина между скоростью и надёжностью.
- **4B** — **рекомендуемый минимум для продакшена.** Проходит **ВСЕ** PlayMode тесты CoreAI. Баланс для **AINpc**, отчётов и большинства задач.
...
- **Gemma 4 26B** (через LM Studio / HTTP API) — отлично работает со всеми тестами.
...
Это помогает маленьким моделям (Qwen3.5-0.8B/2B) которые иногда забывают формат tool call.
...
| 1.3 | §6.6 Qwen 3.5 2B/4B/9B и OpenAI-compatible через CoreAI |
| 1.4 | §8 валидация ответов LLM per role, retry flow, кастомные роли |
| 1.5 | §1.3.5 Universal System Prompt Prefix — универсальный стартовый промпт для всех агентов |
| 1.6 | §6.6 добавлены 0.8B и Gemma 4 26B; обновлены результаты тестирования по размерам моделей |
