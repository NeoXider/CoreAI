# 📋 Формат команд (Tool Calls) под капотом MEAI

**Версия документа:** 2.0 | **Дата:** Апрель 2026

Этот документ описывает **внутренний формат** команд (tool calls), которые генерируются моделями при использовании функций. С переходом на нативный MEAI (Microsoft.Extensions.AI) function calling, этот формат управляется автоматически.

---

## Общий формат Tool Call (Внутренняя структура)

Под капотом, когда модель решает вызвать инструмент, формируется структура, эквивалентная следующему MEAI формату:
...
> [!WARNING]
> **НЕ УКАЗЫВАЙТЕ ЭТОТ ФОРМАТ В ПРОМПТАХ!** 
> Нативный MEAI клиент автоматически передает схему инструментов (JSON Schema) в модель. Если вы вручную попросите модель вернуть JSON-текст, она ответит обычным текстом, содержащим JSON, вместо того чтобы триггернуть системный (нативный) вызов инструмента. В ваших промптах просто говорите: "Используй инструмент [имя]".
