...
| `GameLogSettingsAsset` | CoreAiUnity | `IGameLogSettings`, `GameLogSettingsOptions`, `ToOptions()`, `ApplyOptions(...)` | `GameLogFeature` and `GameLogLevel` are portable because they do not depend on Unity. The asset is authoring-only: the container copies it into `GameLogFilter` while building, and the game filters against that runtime copy — so play-mode changes never write back into the `.asset`. |
