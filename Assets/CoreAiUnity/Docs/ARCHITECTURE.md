...
The legacy primary client gains `FallbackLlmClientDecorator` when the secondary backend is configured. `CircuitBreakerLlmClientDecorator` (portable core) is not part of the default Unity chain; a host that wants it composes it around its own client.
...
`IAiPromptContextProvider` lets a game append per-request context such as current quest, lesson slot, learner profile, or world objective without mutating the static role prompt. `AiOrchestrator` sends it as the final `## World State` entry of the ordered tail in `LlmCompletionRequest.ChatHistory`; memory, pending memory updates and the conversation summary travel in the same tail, never in the cacheable system prefix (see [DEVELOPER_GUIDE §3.5](DEVELOPER_GUIDE.md#35-prompt-layers-what-the-model-actually-sees)). The older `AiPromptComposer.AppendRuntimeContext` helper (`## Runtime Context`) is not used by the orchestrator.
...
```csharp
public sealed class StudentMemoryScopeProvider : AgentMemoryScopeProviderBehaviour
{
    ...
    {
        return new AgentMemoryScope("my-school", CurrentStudentId, "", "");
    }
}
...
`FileAgentMemoryStore` still implements `IConversationTranscriptStore`. Its on-disk layout (v2) is two files per key: `<stem>.json` (memory document, versions, system-prompt snapshot) and `<stem>.history.jsonl` (one JSON line per chat/transcript record), where `<stem>` is the role id or `scope-v1-<sha256>`. Legacy `chatHistoryJson` / `transcriptEntriesJson` fields inside the memory document are migrated on first access. `InMemoryAgentMemoryStore` implements the same memory/chat/transcript and atomic-mutation capabilities without filesystem API. The selected production backing is never published directly as either memory or transcript, so the raw role key cannot be bypassed via a DI resolve.

## Timeout & Retry Rule (v1.5.1)

**Timeout:** the interactive UI timeout is enforced by `CoreAiChatService` via UniTask `CancelAfterSlim` (PlayerLoop-based, WebGL-compatible) as an idle deadline (`LlmRequestTimeoutSeconds`, re-armed by tool activity and stream chunks). The service keeps the caller's token and the deadlines apart: `AiTaskRequest.CallerCancellationToken` is the caller's token only, `AiTaskRequest.DeadlineCancellationToken` is the idle deadline joined with an optional host deadline (the `SendMessageAsync` / `SendMessageResultAsync` / `SendMessageStreamingAsync` overloads that take a `deadlineToken`), and the operation token links all of them. A deadline that fires while the caller is alive ends the turn with `LlmOperationTimeoutException` (streaming and buffered) and is recorded as `AiLlmCompletionOutcome.DeadlineCancellation`; a caller stop stays a cancellation even if a deadline fired too. Since **5.4.0** the portable pipeline also wraps every client in `TimeoutLlmClientDecorator`, which applies a request timeout off `ICoreAISettings.LlmRequestTimeoutSeconds` on both the streaming and non-streaming paths (belt-and-braces for hosts that call the pipeline outside the chat service). In Unity, `LlmPipelineInstaller` injects `UnityMainThreadLlmAsyncMarshaler`, so the portable deadline is also driven by the PlayerLoop instead of `CancellationTokenSource.CancelAfter`. See [`STREAMING_ARCHITECTURE.md`](STREAMING_ARCHITECTURE.md) §8.

**Retries:** network-level retries (HTTP 429, 5xx, exponential backoff) are handled exclusively by `LoggingLlmClientDecorator`. The orchestrator does not multiply those retries with its own counters. Since **5.4.0** `RetryingStreamingLlmClientDecorator` additionally retries a *streaming* call, but only before the stream commits any content, so a mid-stream failure is never silently restarted. Both decorators receive the Unity marshaler for PlayerLoop-driven retry delays in WebGL. `LlmPipelineInstaller` composes the chain as `Timeout( Logging( RetryingStreaming( routed ) ) )`. No retry or fallback decorator replays a request after its caller cancelled; the fault propagates as that cancellation.
...
**Timeout vs cancellation.** `LlmCancellation` (portable core) decides how an interrupted request is classified, and every layer applies the same rule:

- **Caller token cancelled:** every fault is the caller's cancellation (`LlmErrorCode.Cancelled`, error text `cancelled`) — an `OperationCanceledException`, a library timeout, a typed `LlmClientException`, an untyped transport fault, a failed result or a terminal error chunk alike. This holds for the user-facing code, the completion metrics and `CircuitBreakerLlmClientDecorator` (which records no verdict and releases its half-open probe slot). A layer that must throw after a caller cancel throws an `OperationCanceledException` with the fault attached as `InnerException` (`LlmCancellation.WrapAsCancellation`; `LlmCancellation.FindClientException` finds it again).
- **Caller token alive:** `LlmOperationTimeoutException` (also inside an `AggregateException`) is a `Timeout`; any other `OperationCanceledException` is `Cancelled` (someone else stopped the work: `CoreAi.StopAgent`, a scope, a disposed orchestrator); anything else is classified by the layer itself (typed code, else `ProviderError`).
- **Metrics:** a terminal timeout — thrown, or a `Timeout` chunk from `TimeoutLlmClientDecorator` on the streaming path — is recorded as `DeadlineCancellation`; a library timeout after the caller already cancelled counts as the caller's cancellation.
- **Endpoint health** is the one consumer that still looks at the raw fault (see [RUNTIME_BACKEND_SWITCHING](RUNTIME_BACKEND_SWITCHING.md)): a permanent refusal (`AuthExpired`, `PaymentRequired`) is reported even when a caller cancel raced it; transport, timeout and provider faults after a caller cancel are not.

In the chat panel a turn that ends interrupted goes to exactly one of two hooks: `ResolveTimeoutMessage(false)` for a library timeout (`LlmOperationTimeoutException`, or a terminal `LlmErrorCode.Timeout` chunk while the caller's token was alive) or a fired host deadline, and `ResolveCancelledMessage()` for every cancellation — the external caller's token, the panel's own stop source, or `CoreAi.StopAgent` from elsewhere. The default cancelled text is `null`, so a cancelled turn adds nothing to the transcript; a UI Stop reaches neither hook. The log line is `[CoreAiChatPanel] Turn interrupted (reason=timeout|cancelled)`. `SubmitMessageFromExternalResultAsync` reports the same split in `Completion.ErrorCode`. A host that runs its own deadline passes it as `CoreAiChatExternalSubmitOptions.DeadlineCancellationToken`: the panel hands it to `CoreAiChatService` separately, so it stops the turn and is reported as a timeout — in the panel and in the metrics — while the caller's token is alive. A deadline armed on the caller's token itself is indistinguishable from a stop and is reported as a cancellation. An already-elapsed deadline admits nothing: the string API `SubmitMessageFromExternalAsync` adds no user bubble, raises no `OnUserMessageSent`, makes no service call, shows `ResolveTimeoutMessage(false)` once and returns `null`; with an already-cancelled caller it does nothing at all. The typed API rejects both before admission with `Timeout` / `Cancelled`. When a hook runs, the base class has already released the busy flags, the typing indicator and the streaming bubble.
...
**Lua on WebGL player:** runtime Lua execution through **`LuaCsSecureEnvironment`** (Lua-CSharp, `com.neoxider.coreaimods`) is supported on WebGL player builds whenever `COREAI_LUA` is compiled in. `ICoreAISettings.EnableLuaOnWebGl` (serialized on `CoreAISettingsAsset`, default `true`) is currently not read by the Lua runtime. The Full `unity_*` reflection tier is granted only by `CoreAiModsLifetimeScope.FullLuaAccessEnabled`; keep it off in browser builds. Lua-CSharp is a managed, AOT-safe VM, so no IL2CPP stripping protection is needed for it.
