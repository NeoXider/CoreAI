...
**Async continuations (v1.5.9 — both `com.nexoider.coreai` and `com.nexoider.coreaiunity`):** The non-streaming MEAI path (`SmartToolCallingChatClient`, `AiOrchestrator._llm.CompleteAsync`, `MeaiLlmClient.GetResponseAsync`, `MeaiOpenAiChatClient` UnityWebRequest poll) uses **`ConfigureAwait(false)`** and avoids **`Task.Yield()`** in the tool loop so continuations are less likely to stall on the single-threaded player loop. **`CoreAiChatPanel`** may still **`Task.Yield()`** once before streaming to let the UI repaint — that is UI-specific, not the HTTP/tool stack.
