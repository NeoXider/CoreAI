...
**Lua on WebGL player:** runtime Lua execution through **`SecureLuaEnvironment`** / MoonSharp is temporarily disabled in WebGL player builds. **`SecureLuaEnvironment.IsSupported`** returns `false`, and **`LuaAiEnvelopeProcessor`** publishes **`LuaExecutionFailed`** instead of instantiating MoonSharp. This avoids current WebGL/IL2CPP crashes from MoonSharp reflection-based loader initialization. Lua remains supported in Editor and non-WebGL players. Future WebGL support can be restored with an AOT-safe Lua runtime, server-side Lua execution, or a restricted command interpreter, covered by WebGL Player tests.
