...
**HTTP LLM on WebGL player:** Unity forbids `System.Net` / `HttpClient` in browser builds. By default **`MeaiLlmClient.CreateHttp`** uses **`UnityWebRequestOpenAiTransport`** when **`UNITY_WEBGL && !UNITY_EDITOR`** — it does not deliver incremental SSE; **`MeaiOpenAiChatClient`** may complete with **`stream: false`** and **simulate** streaming updates. When **`CoreAISettingsAsset.WebGlNativeStreaming`** is enabled, **`FetchSseOpenAiTransport`** + **`CoreAiSseFetch.jslib`** use **`fetch`** for real SSE (see [`HTTP_TRANSPORT_SPEC.md`](HTTP_TRANSPORT_SPEC.md)). Cross-origin APIs need **CORS** (see [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) § WebGL).
