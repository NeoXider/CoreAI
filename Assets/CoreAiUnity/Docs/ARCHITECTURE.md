...
**HTTP LLM on WebGL player:** Unity forbids `System.Net` / `HttpClient` in browser builds. **`MeaiLlmClient.CreateHttp`** uses **`UnityWebRequestOpenAiTransport`** when **`UNITY_WEBGL && !UNITY_EDITOR`**. That transport does not support SSE; **`MeaiOpenAiChatClient`** completes with **`stream: false`** and **simulates** streaming updates. The API endpoint must send **CORS** headers the browser accepts (see [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) § WebGL). Spec: [`HTTP_TRANSPORT_SPEC.md`](HTTP_TRANSPORT_SPEC.md).
