...
**Why LLMUnity's own `Overflow Strategy` is forced to `None` (since v5.0.6):** `LLMAgent` ships a `ContextOverflowStrategy` (`None` / `Truncate` / `Summarize`) that trims or summarizes the agent's *internal* chat history when it would exceed the context window. CoreAI never uses that history — the runtime client (`LlmUnityMeaiChatClient`) builds the **entire** prompt itself (system + one pre-assembled user message that already contains the `## Conversation Summary` and recent turns) and calls **`Chat(addToHistory: false)`**. So the agent's internal history is always empty and LLMUnity's overflow strategy has nothing to act on — it is a silent no-op whose Inspector default (`Truncate`) misleadingly implies LLMUnity manages the context. Both `LlmUnityHostConfigurator.ApplyFromSettings` (runtime auto-create) and `CoreAI/Setup/Create LLMUnity Objects` (Editor) now explicitly set `agent.overflowStrategy = ContextOverflowStrategy.None`, so context management lives **only** in CoreAI's backend-agnostic compaction (the same behavior you get on the HTTP/OpenAI path, where the server never manages history either). Do not re-enable it unless you also switch CoreAI to `addToHistory: true` and stop building the prompt yourself — which would disable role-aware, persisted, backend-independent summarization.
