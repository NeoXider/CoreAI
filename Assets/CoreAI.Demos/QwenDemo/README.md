...
5. In Spellcraft, use `Determinism x5` to repeat the same prompt with temperature `0` and inspect the decision distribution.
...
0.8B model explicit RU/EN element aliases; for example, the Russian input `мега молния` (quoted
verbatim) maps to `storm` power `3`.
...
The release smoke also checks the actual tool arguments for the Russian element presets (Russian preset
inputs, quoted verbatim). The compact model must return `storm|3` for `мега молния`, `fire|2` for `стена огня`, `poison|1` for
