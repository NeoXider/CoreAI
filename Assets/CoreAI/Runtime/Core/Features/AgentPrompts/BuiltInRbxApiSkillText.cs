...
- `game:GetService(""Name"")` — the standard runtime has live tree-backed `Workspace`, `Lighting`,
  ...
  `HttpService`, `UserInputService`, `RunService`, and `MaterialService` services.
- The catalog's 13 registrations are: `HttpService` and `Players` (tree-backed), `RunService`
  (MVP2; fallback), `UserInputService` (MVP10; fallback), `TweenService` (MVP8),
  `CollectionService` (MVP8), `Debris` (MVP8), `DataStoreService` (MVP9),
