...
namespace CoreAI.Ai
{
    /// <summary>Парсинг fenced-блоков <c>memory</c> / <c>memory_append</c> / <c>memory_clear</c> в ответе модели.</summary>
    public static class AgentMemoryDirectiveParser
    {
        /// <summary>Нормализованная директива для <see cref="AiOrchestrator"/> и хранилища памяти.</summary>
        public sealed class MemoryDirective
        {
            /// <summary>Полная очистка памяти роли.</summary>
            public bool Clear { get; set; }

            /// <summary>Дописать к существующей памяти вместо замены.</summary>
            public bool Append { get; set; }

            /// <summary>Текст для сохранения (пусто при только <see cref="Clear"/>).</summary>
