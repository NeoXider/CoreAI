...
- `LocalModel` вЂ” local model adapter, such as LLMUnity or a future non-Unity local runtime.
- `ClientOwnedApi` вЂ” direct OpenAI-compatible API with a key owned by the user or application developer.
- `ClientLimited` вЂ” client path with local or server-enforced request/prompt limits.
- `ServerManagedApi` вЂ” production backend/proxy owns provider keys, subscriptions, quotas, model allowlists, usage, and audit.
- `Offline` / `Stub` вЂ” deterministic fallback for tests and demos.
...
- `LlmUsageRecord` and `ILlmUsageSink` provide portable usage accounting **contracts**. Free CoreAI does not register a default sink вЂ” use a custom adapter or rely on the backend to record usage. CoreAiPro ships a backend `BackendUsageSink` adapter.
- `ILlmEntitlementPolicy` and `LlmEntitlementDecision` provide portable subscription/quota/allowlist **contracts**. Free CoreAI does not run a client-side entitlement decorator вЂ” the backend (`ServerManagedApi`) is the source of truth and surfaces decisions through `LlmErrorCode.QuotaExceeded` / `RateLimited` / etc. CoreAiPro ships a backend `BackendEntitlementPolicy` adapter that calls `GET /entitlements`.
