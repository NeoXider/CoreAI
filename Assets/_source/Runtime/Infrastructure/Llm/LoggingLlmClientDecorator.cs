    ...
    public sealed class LoggingLlmClientDecorator : ILlmClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInfo(GameLogFeature.Llm,
                $"LLM ▶ traceId={trace} role={role} backend={_backendLabel}\n" +
            ...
            using (var linked = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
            {
                ...
                catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    _logger.LogWarning(GameLogFeature.Llm,
                        $"LLM ⏱ traceId={trace} role={role} backend={_backendLabel} wallMs={sw.Elapsed.TotalMilliseconds:F0} | {msg}");
            ...
            {
                _logger.LogWarning(GameLogFeature.Llm,
                    $"LLM ✖ traceId={trace} role={role} backend={_backendLabel} wallMs={wallMs:F0} | результат null");
                ...
                string.Equals(result.Error, "Cancelled", StringComparison.Ordinal))
            {
                var msg = $"LLM request timeout ({_requestTimeoutSeconds}s)";
                _logger.LogWarning(GameLogFeature.Llm,
                    $"LLM ⏱ traceId={trace} role={role} backend={_backendLabel} wallMs={wallMs:F0} | {msg}");
                return new LlmCompletionResult { Ok = false, Error = msg };
            }
            ...
            {
                _logger.LogWarning(GameLogFeature.Llm,
                    $"LLM ✖ traceId={trace} role={role} backend={_backendLabel} wallMs={wallMs:F0} | {result.Error ?? "(без текста)"}");
                return result;
            }
            ...
            _logger.LogInfo(GameLogFeature.Llm,
                $"LLM ◀ traceId={trace} role={role} backend={_backendLabel} wallMs={wallMs:F0} | {tokLine}\n" +
