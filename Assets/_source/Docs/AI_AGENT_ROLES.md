...
| **Поведение** | На основе снимка мира и целей дизайна выдаёт **структурированные** пакеты (JSON/таблицы): новые аффиксы, смена фазы, постановка задач другим агентам. |
...
| **Выходы** | Подписанный **UseCaseScript** (строка/hash) + команда «подключить/заменить» через шину. |
...
Ядро `_source` должно предлагать **готовые строительные блоки** (подписки / агрегаторы), а игра — включать нужные:
