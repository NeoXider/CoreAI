...
namespace CoreAI.ExampleGame.ArenaSurvival.View
{
    public sealed class ArenaSurvivalHud : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            string h = _health != null ? $"{_health.Current} / {_health.Max}" : "?";
            string prog = "";
            if (_progressionTeam != null && _sessionCurve != null)
            {
                int next = _sessionCurve.GetXpToNextLevel(_progressionTeam.SessionTotalXp);
                prog =
                    $"  |  Ур. {_progressionTeam.SessionLevel}  XP {_progressionTeam.SessionTotalXp} (до след. {next})";
            }
