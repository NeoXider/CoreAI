...
**Version:** **v0.25.3** · chat hotkeys (C / Esc, настраиваемые в `CoreAiChatConfig`) · header buttons cleanup для WebGL (emoji `⏹` → `■` в `CoreAiChat.uxml` — пустые кнопки в браузере исправлены) · WebGL `TextField` focus persistence · cross-input-system compatibility (Legacy + new Input System Package) · `ForcedToolMode` (deterministic tool_choice) · multi-round streaming tool loop · `CoreAiChatPanel.BuildAiTaskRequest` extension hook · **TODO** [`STREAMING_WEBGL_TODO`](Assets/CoreAiUnity/Docs/STREAMING_WEBGL_TODO.md) — план фикса WebGL SSE-стриминга в `MeaiOpenAiChatClient.CompleteStreamingAsync`
