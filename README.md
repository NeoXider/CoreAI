...
**Current stable line:** Core portable **`1.5.5`**; CoreAI Unity **`1.5.6`** — optional **LLM-assisted context compaction**, context budget / summaries, **`1.5.6`** WebGL/async **main-thread marshaling** for `CoreAiChatPanel` (streaming + non-streaming cleanup), **1.5.5** PlayMode tests for non-streaming external submit, plus **1.4.x** HTTP `Retry-After`, `TryRepairToolName`, unified tool pipeline. Version source: [`Assets/CoreAiUnity/package.json`](Assets/CoreAiUnity/package.json) · [`Assets/CoreAI/package.json`](Assets/CoreAI/package.json).
