...
- **(a) Which metric character controller backs `Humanoid`.** The roadmap's "existing CoreAI player/controller" does not exist in the CoreAI packages; candidates are NeoxiderTools' `CharacterMovementFundamentals` sample, the example game's `ArenaPlayerMotor`, or a minimal new `RbxCharacterMotor`. Slice 8.6 is blocked on this; 8.0–8.5 are not.
...
- **(g) `Humanoid` passive regeneration** (S3.1: 1 % MaxHealth/s unless a `Health` script exists). Implement, or record `DEV-13` as a conscious deviation. Kill-brick fixtures are indifferent; damage-over-time fixtures are not.
